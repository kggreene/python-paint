#Hi Jon
#I'm heading out of town for a week, back on Dec. 9. I didn't have time to get everything rewritten 
#but here's a chunk if you want to start looking at it.

#An A is a black and white sketch and a B is the colored version
#We'll start with ~30 black and white sketches (A's) and are trying to create 400-1000 AB pairs to use for a TensorFlow model
#Class SketchComponents stores a black and white sketch A and colors it by geometric rules 
#(which work on 'rule conforming' sketches) to get B.
...
class Augmentations():
    ...
    #randxyScaledSkewedPair, randTranslationPair, randRotationPair, mirrorFlipPair, radiusCubedPair (r->r**3), 
    #elasticTransformColorSetPair (Gaussian filters)
    
    #still need elasticDeformationGray (done for one img but need set version? incorporate read BW, transform, color into the function?) 
    ...
    def randxyScaledSkewedPair(self, imgA,imgB):
        ...
        size=len(imgA)
        xscale=np.random.uniform(.8,1)
        yscale=np.random.uniform(.8,1) 
        skewFactor=np.random.uniform(0,.1)
        M = np.float32([[xscale,0,0],[skewFactor,yscale,0]])
        scaleSkewA = cv2.warpAffine(imgA, M, (size, size), borderMode=cv2.BORDER_CONSTANT, borderValue=(255,255,255))
        scaleSkewB = cv2.warpAffine(imgB, M, (size, size), borderMode=cv2.BORDER_CONSTANT, borderValue=(255,255,255))
    ...
    def randTranslationPair(self, imgA, imgB):
        ...
        transA = cv2.warpAffine(imgA, M, (size, size), borderMode=cv2.BORDER_CONSTANT, borderValue=(255,255,255))
        transB = cv2.warpAffine(imgB, M, (size, size), borderMode=cv2.BORDER_CONSTANT, borderValue=(255,255,255))
    ...
    def randRotationPair(self, imgA, imgB, characterType='flower'):
        ...
        rotA = cv2.warpAffine(imgA, M, (size, size), borderMode=cv2.BORDER_CONSTANT, borderValue=(255,255,255))
        rotB = cv2.warpAffine(imgB, M, (size, size), borderMode=cv2.BORDER_CONSTANT, borderValue=(255,255,255))
        return rotA, rotB
    ...
    def elasticTransformColorSetPair(self, imgA, imgB):
        ...
        #Creates a list of four transformed AB pairs with different parameters for each pair
        alphas = [50, 200, 800, 1500]
        sigmas = [3, 4, 7, 8]
        Alist=[]
        Blist=[]
        for i in range(len(alphas)):
            transA, transB = self.elasticTransformColorPair(imgA, imgB, alphas[i], sigmas[i])
            Alist.append(transA)        
            Blist.append(transB)
        return Alist, Blist

    def elasticTransformColorPair(self, imgA, imgB, alpha, sigma, random_state=None):
        #function for elasticTransformColorSetPair
        #elastic deformations/random displacement fields
        #modified from Github https://gist.github.com/erniejunior/601cdf56d2b424757de5; based on Simard, et al
        ...
        distorted_imgA = map_coordinates(imgA, indices, order=1, mode='reflect')
        distorted_imgB = map_coordinates(imgB, indices, order=1, mode='reflect')
    ...
    def elasticDeformationGray(self, img, alpha, blurSize):
        ...
        #so unlike the AB pair transformations, read in as grayscale, transform and change to BGR, then color A' to get B'
        #function for elasticDeformationGrayPairSet
        #similar to elasticTransformColor but different parameters and results turning out enough different worth using
        #good parameters:
        ...
        distorted_image = map_coordinates(img, indices, order = 1, mode='constant', cval=255)
    ...
    def radiusCubedPair(self, imgA, imgB):
        ...
        #homeomorphism of unit disk shrinks center disproportionally; based on r->r**3
    ...
    def radiusCubed(self, imgA, scale=.5):
        ...
        #translate and scale to fit in unit disk; r, theta ->r**3, theta; rescale, translate to original square
...
class SketchComponents():
    #Part of project DragonPaint
    #SketchComponents stores sketch image and uses component information (area, color, bounding box) to 
    ...
    def setEyeSpikes(self):
        ...
        #If "spike"'s puffed up bounding box intersects the background it's a spike; if not, it's an eye (or part of an eye)
        
        linePlus=5
        #linePlus gives a puffed up bounding box to account for drawing line thickness; it may need to be adjusted if 
        #pictures have been scaled down from original (400x400 image with 5 pixel line.) 
        ...
        newSpikeLabels=[label for label in self.spikeLabels if label not in eyeLabels]
        return eyeLabels, newSpikeLabels


#TEST CODE FOR SKETCHCOMPONENTS
#run line with "Flower1.png" to see flower or the line with "Dragon4.png" to see dragon
